...
select p.game_id, p.hteam_id, CASE WHEN bx.winner_home_or_away = 'H' THEN 1 ELSE 0 END AS HomeTeamWins,
b.h_ttol_bases,b.a_ttol_bases  ,p.h_roll_strikeout, p.a_roll_strikeout, p.h_roll_hit , p.a_roll_hit ,bx.home_runs, bx.away_runs, bx.home_hits,
bx.away_hits  from boxscore bx
left join startingpitcher_rolling_stats p on bx.game_id = p.game_id 
left join batting_stats_rolling_stats b on p.game_id = b.game_id order by bx.game_id);
