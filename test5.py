...
class response_table:
    ...
    def get_feature_importance(self, pred_var, response):
        X = sm.add_constant(
            self.df[pred_var]
        )  # Added a constant term to the X variable
    ...
    def random_variable(self, cont_var, response):
        ...
        feat_rank = feat_rank.sort_values("impact", ascending=False).reset_index(
            drop=True
        )
...
def plotting_cont(df, pred_var, response):
    ...
    bin_1 = (
        len(data_response[pred_var].unique())
        if len(data_response[pred_var].unique()) < 10
        else 10
    )
...
def cat_cont_res(df, pred_var, response):
    ...
    population_a, _ = df[pred_var].value_counts(), [
        i for i in range(0, len(df[pred_var].unique()))
    ]
    ...
    fig.update_layout(
        title_text="{pred_var} vs {response} Mean of Response Rate Plot",
        legend=dict(orientation="v"),
        yaxis=dict(
            title=dict(text="Total Population"),
            side="right",
...
def DisplayTableTemplate(df, pred_var, response, is_cont=False):
    ...
    if is_cont:
        ...
        fig.update_layout(
            ...
            xaxis_title="Groupings (survived)",
        )
...
def continuous_continuous_pairs(df, cont_var):
    ...
    temp_df = pearson_df.merge(
        url_df, left_on="cont_1", right_on="predictor", how="left"
    )
...
def create_table(final_table, file_name, clickable, sort_by):
    ...
    table = final_table.style.set_properties(
        **{"border": "1.6px dotted purple"}
    ).format(plot_click, escape="html")
...
def mean_of_response_cont_cont(X, continuous, response):
    ...
    for i in range(0, len(continuous)):
        ...
        diff_mean_unweighted = (res_means_diff_df.pow(2).sum() / bin_size_2d) ** 0.5
        diff_mean_weighted = (
...
def mean_res_cat(df, cat_var, response):
    cat_diff_table = pd.DataFrame(
        columns=[
            "predictor",
            "Diff Mean Response (Weighted)",
            "Diff Mean Response (Unweighted)",
        ]
    )
    for p in cat_var:
...
def main(logger):
    ...
    cont_temp_table["file_link"] = cont_temp_table["file_link"].apply(
        lambda x: os.path.join(os.getcwd(), x)
    )
    global cont_url_dict
    cont_url_dict = {
        row["predictor"]: [row["file_link"]] for i, row in cont_temp_table.iterrows()
    }
    data = pd.DataFrame(temp_df)
    output = mean_of_response_cont_cont(df, cont_var, response)
    mean_of_res = pd.DataFrame(output)
    first_table = (
        data.merge(random_score, left_on="col", right_on="columns", how="inner")
        .merge(mean_of_res, left_on="columns", right_on="column", how="inner")
        .drop(["columns", "column"], axis=1)
    )
    first_table = first_table[
        ["col", "weigh", "un_weigh", "impact", "p_value", "t_value"]
    ]
    first_table = first_table.rename(
        columns={
            "col": "Feature",
            "weigh": "Diff Mean Response (Weighted)",
            "un_weigh": "Diff Mean Response (Unweighted)",
            "impact": "Random Forest Variable Importance",
            "p_value": "P-Value",
            "t_value": "T-Score",
            "impact": "Random Forest Variable Importance",
        }
    ...
    table["file_link"] = table["file_link"].apply(
        lambda x: os.path.join(os.getcwd(), x)
    )
    cont_temp_table = table.merge(cont_temp_table, on="predictor", how="inner").merge(
        first_table, left_on="predictor", right_on="Feature", how="inner"
    )
    cont_temp_table.rename(
        columns={"file_link_y": "Mean of Response Plot", "file_link_x": "Plot"},
        inplace=True,
    )
    ...
    for i in cat_var:
        ...
        temp_table["file_link"] = temp_table["file_link"].apply(
            lambda x: os.path.join(os.getcwd(), x)
        )
    ...
    cat_temp_table.rename(
        columns={"file_link_x": "Plot", "file_link_y": "Mean of Response Plot"},
        inplace=True,
    )
    ...
    create_table(
        second_table, "cat_plot.html", click2, sort_by="Diff Mean Response (Weighted)"
    )
    ...
    url_df = pd.DataFrame(cat_url_dict).T.reset_index()
    url_df.columns = ["predictor", "file_links"]

    url_df1 = pd.DataFrame(cont_url_dict).T.reset_index()
    url_df1.columns = ["predictor", "file_links"]

    cramer_df = pd.DataFrame(columns=["cat_1", "cat_2", "corr"])
    k = 0
    for i in cat_var:
        for j in cat_var:
            if i != j:
                corr = co_rel.cat_correlation(df[i], df[j])
                cramer_df.loc[k] = [i, j, corr]
                k += 1
    temp_df = cramer_df.merge(url_df, left_on="cat_1", right_on="predictor", how="left")
    url_df.rename(columns={"file_links": "links"}, inplace=True)
    temp_df = temp_df.merge(url_df, left_on="cat_2", right_on="predictor", how="left")
    temp_df.drop(["predictor_y", "predictor_x"], axis=1, inplace=True)

    temp_df.rename(
        columns={"file_links": "cat_1_url", "links": "cat_2_url"}, inplace=True
    )
    click5 = ["cat_1_url", "cat_2_url"]
    create_table(temp_df, "cramer_corr.html", click5, sort_by="corr")
    ...
    fig.update_layout(
        title="Variable: cramer_corr",
        yaxis_title="category 1",
        xaxis_title="categorgy 2",
    )
    filename = "cramer_corr_plot.html"
    fig.write_html(file=filename, include_plotlyjs="cdn")

    del temp_df

    tschuprow_df = pd.DataFrame(columns=["cat_1", "cat_2", "corr"])
    k = 0
    for i in cat_var:
        for j in cat_var:
            if i != j:
                corr = co_rel.cat_correlation(df[i], df[j], tschuprow=True)
                tschuprow_df.loc[k] = [i, j, corr]
                k += 1

    temp_df = tschuprow_df.merge(
        url_df, left_on="cat_1", right_on="predictor", how="inner"
    )
    url_df.rename(columns={"file_links": "links"}, inplace=True)
    temp_df = temp_df.merge(url_df, left_on="cat_2", right_on="predictor", how="inner")
    temp_df.drop(["predictor_y", "predictor_x"], axis=1, inplace=True)

    temp_df.rename(
        columns={"file_links": "cat_1_url", "links": "cat_2_url"}, inplace=True
    )
    create_table(temp_df, "tschuprow_corr.html", click5, sort_by="corr")
    ...
    fig.update_layout(
        title="Variable: tschuprow_corr",
        yaxis_title="category 1",
        xaxis_title="categorgy 2",
    )
    filename = "tschuprow_corr_plot.html"
    fig.write_html(file=filename, include_plotlyjs="cdn")
    del temp_df

    correlation_ratio = pd.DataFrame(columns=["cat", "cont", "corr"])
    k = 0
    for i in cat_var:
        for j in cont_var:
            corr = co_rel.cat_cont_correlation_ratio(df[i], df[j])
            correlation_ratio.loc[k] = [i, j, corr]
            k += 1

    temp_df = correlation_ratio.merge(
        url_df, left_on="cat", right_on="predictor", how="inner"
    )
    url_df1.rename(columns={"file_links": "links"}, inplace=True)
    temp_df = temp_df.merge(url_df1, left_on="cont", right_on="predictor", how="inner")
    temp_df.drop(["predictor_y", "predictor_x"], axis=1, inplace=True)
    click3 = ["cat_1_url", "cont_2_url"]
    temp_df.rename(
        columns={"links_x": "cat_1_url", "links_y": "cont_2_url"}, inplace=True
    )
    create_table(temp_df, "correlation_ratio.html", click3, sort_by="corr")
    fig = go.Figure(
        data=go.Heatmap(
            y=temp_df["cat"],
            x=temp_df["cont"],
    ...
    fig.update_layout(
        title="Variable: correlation_ratio",
        yaxis_title="category 1",
        xaxis_title="categorgy 2",
    )
    filename = "correlation_ratio_plot.html"
    fig.write_html(file=filename, include_plotlyjs="cdn")
    ...
    cat_cat_brute_force_table = pd.DataFrame(
        columns=[
            "cat_1",
            "cat_2",
    ...
    cat_cont_brute_force_table = pd.DataFrame(
        columns=[
            "cat",
            "cont",
            "diff_mean_resp_ranking",
    ...
    for c1 in cont_var:
        for c2 in cont_var:
            ...
            if c1 != c2:
                ...
                length = {y: np.size}
                mean_values = (
                    df.groupby([c1_binning, c2_binning]).agg(mean).reset_index()
                )
                lengths = df.groupby([c1_binning, c2_binning]).agg(length).reset_index()
                ...
                t1 = [
                    round(i, 4)
                    for i in mean_values_merged["diff with mean of response"].tolist()
                    if i not in [np.nan, float("Nan")]
                ]
                fig = go.Figure(
                    data=go.Heatmap(
    ...
    cat_cat_brute_force_table_temp = pd.DataFrame()
    # Cat-cat brute force
    for c1 in cat_var:
        for c2 in cat_var:
            if c1 != c2:
                ...
                df_temp[c1_binning] = df[c1]
                df_temp[c2_binning] = df[c2]

                mean = {y: np.mean}
                length = {y: np.size}

                mean_values = (
                    df_temp.groupby([c1_binning, c2_binning]).agg(mean).reset_index()
                )
                lengths = (
                    df_temp.groupby([c1_binning, c2_binning]).agg(length).reset_index()
                )
                ...
                # print(mean_values_merged[p1+'_x'],mean_values_merged[p1+'_y'])
                ...
                mean_values_merged["squared diff"] = (
                    1
                    / (len(set(df_temp[c1])) * len(set(df_temp[c2])))
                    * np.power(mean_values_merged["diff with mean of response"], 2)
                )
                ...
                fig.write_html(file=filename, include_plotlyjs="cdn")
                try:
                    cramer_corr = co_rel.cat_correlation(df_temp[c1], df_temp[c2])
                except Exception:
                    cramer_corr = 0

                try:
                    tschuprow_corr = co_rel.cat_correlation(
                        df_temp[c1], df_temp[c2], tschuprow=True
                    )

                except Exception:
                    tschuprow_corr = 0
                cat_cat_brute_force_table.loc[len(cat_cat_brute_force_table)] = [
                    c1,
                    c2,
                    unweigh_diff,
                    weigh_diff,
                ...
                cat_cat_brute_force_table_temp = pd.concat(
                    [cat_cat_brute_force_table_temp, cat_cat_brute_force_table], axis=0
                ).reset_index(drop=True)
    create_table(
        cat_cat_brute_force_table_temp,
        "brute_force_cat_cat.html",
        ["link"],
        sort_by="cramer",
    )
    # print(cat_cat_brute_force_table.to_csv("cat_cat_brute_force_table.csv"))
    cat_cont_brute_force_table_temp = pd.DataFrame()
    # Cont - Cat Brute force
    for c1 in cont_var:
        for c2 in cat_var:
            ...
            df_temp[c1_binning] = pd.cut(x=df_temp[c1], bins=10)
            df_temp[c2_binning] = df_temp[c2]
            ...
            mean_values_merged["population_mean"] = df_temp[response].mean()
            mean_values_merged["population"] = mean_values_merged[response + "_y"].sum()
            ...
            # print(pd.DataFrame([p1, p2, DMR_result,DWMR_result]))
            # cont_cont_brute_force_table.loc[len(cont_cont_brute_force_table)] = [p1, p2, DMR_result,DWMR_result ]
            t1 = [
                round(i, 4)
                for i in mean_values_merged["diff with mean of response"].tolist()
            ]
            ...
            filename = f"{c1}_{c2}_cat_cont_brute_force.html"

            fig.write_html(file=filename, include_plotlyjs="cdn")
            try:
                corr_ratio = co_rel.cat_cont_correlation_ratio(df_temp[c2], df_temp[c1])
            except Exception:
                corr_ratio = 0
            cat_cont_brute_force_table.loc[len(cat_cont_brute_force_table)] = [
                c1,
                c2,
                unweigh_diff,
                weigh_diff,
            ...
            try:
                cat_cont_brute_force_table_temp = pd.concat(
                    [cat_cont_brute_force_table_temp, cat_cont_brute_force_table],
                    axis=0,
                ).reset_index(drop=True)
            except Exception:
                print(c1, c2)
    ...
    create_table(
        cat_cont_brute_force_table_temp,
        "brute_force_cat_cont.html",
        ["link"],
        sort_by="corr_ratio",
    )
    # print(cat_cont_brute_force_table.to_csv("cont_cat_brute_force_table.csv"))
