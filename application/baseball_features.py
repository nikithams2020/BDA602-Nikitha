...
def cat_cont_res(df, pred_var, response):
    ...
    population_a, bins = df[pred_var].value_counts(), [
...
def model_build_eval(df, pred, res):
    ...
    X_train, X_test, Y_train, Y_test = train_test_split(
        feat, target, test_size=0.25, shuffle=True, random_state=42
    )

    GBM_CLF = GradientBoostingClassifier(
        n_estimators=150, learning_rate=0.05, max_depth=5, random_state=0
    ).fit(X_train, Y_train)
...
def final_model(df):
    ...
    target = df1[['hometeamwins']]
    feat = df1.drop(['hometeamwins'],axis=1)
    x_train,x_test,y_train,y_test = train_test_split(feat,target,test_size=0.3,random_state=42)
    rf_clf = RandomForestClassifier(random_state=100).fit(x_train,y_train)
    rf_accu = rf_clf.score(x_test,y_test)
    print(f"{rf_accu*100}%")
    top_feat = []
    for e,i in  enumerate(sorted(tuple(zip(rf_clf.feature_importances_,rf_clf.feature_names_in_)))): 
        if e>len(rf_clf.feature_importances_)-11 and i[1] not in ['game_id', 'away_hits', 'home_hits', 'home_error_rate', 'home_runrate' ]: 
            top_feat.append(i[1]) 
    print(top_feat)
    top_feat_data = df1[top_feat+['hometeamwins']]
    target = df1[['hometeamwins']]
    feat = df1.drop(['hometeamwins'],axis=1)
    x_train,x_test,y_train,y_test = train_test_split(feat,target,test_size=0.2,random_state=5431,shuffle=True)

    rf_clf_top = RandomForestClassifier(random_state=100).fit(x_train,y_train)
    rf_top_accu = rf_clf_top.score(x_test,y_test)
    print(f"{rf_top_accu*100}%")

    logit = LogisticRegression(l1_ratio=0.3,solver='saga',penalty='elasticnet').fit(x_train,y_train)
    logit_accu = logit.score(x_test,y_test)
    print(f"{logit_accu*100}%")

    svc = SVC(degree=3,kernel='sigmoid').fit(x_train,y_train)
    svc_accu = svc.score(x_test,y_test)
    print(f"{svc_accu*100}%")

    NN_clf = MLPClassifier(solver='adam',learning_rate_init=0.05,max_iter=500,early_stopping=True,learning_rate='adaptive',activation='relu').fit(x_train,y_train)
    nn_accu = NN_clf.score(x_test,y_test)
    print(f"{nn_accu*100}%")

    print(classification_report(y_test,rf_clf_top.predict(x_test)))
    from sklearn.metrics import confusion_matrix
    from sklearn.metrics import ConfusionMatrixDisplay

    # y_pred = rf_clf_top.predict(x_test)
    # cm = confusion_matrix(y_test, y_pred)

    # cm_display = ConfusionMatrixDisplay(cm).plot()
    fpr, tpr, thresholds = roc_curve(y_test,rf_clf_top.predict(x_test))
    roc_auc = auc(fpr, tpr)
...
def main(logger):
    ...
    query = """ SELECT * FROM base_features  """
    df = pd.read_sql(query,con=sql_engine)
    df.fillna(0, inplace=True)
    #df = df.drop(['game_id'],axis=1)
    top_feature = final_model(df)
    df = df[top_feature+['hometeamwins']] 
    ...
    if len(cat_var) > 0:
        ...
        for i in cat_var:
            ...
            cat_temp_table_1 = cat_temp_table_z.merge(
                table, on="predictor", how="inner"
            )
        ...
        second_table = cat_temp_table.merge(
            mean_of_res_cat, on="predictor", how="inner"
        )
