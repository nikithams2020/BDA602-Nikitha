...
def getfilename(a):
    db_user = "admin"
    db_pass = "1234"  # pragma: allowlist secret
    db_host = "localhost"
...
def model_build_eval(df,pred,res):
    ...
    GBM_CLF = GradientBoostingClassifier(n_estimators=150, learning_rate=0.05,max_depth=5, random_state=0).fit(X_train, Y_train)
...
def main(logger):
    ...
    df = pd.DataFrame(query, con=sql_engine)
    ...
    first_table = data.merge(random_score, left_on='col',right_on='columns',how='inner' ).merge(mean_of_res, left_on ='columns',right_on='column',how='inner' ).drop(['columns','column'],axis=1)
    first_table = first_table[['col','weigh','un_weigh','impact','p_value','t_value']]
    first_table = first_table.rename(columns = {'col':'Feature','weigh':"Diff Mean Response (Weighted)",'un_weigh':"Diff Mean Response (Unweighted)",'impact':'Random Forest Variable Importance','p_value':'P-Value','t_value':'T-Score','impact':'Random Forest Variable Importance'})
    ...
    cont_temp_table = table.merge(cont_temp_table, on='predictor',how='inner').merge(first_table,left_on='predictor',right_on='Feature',how='inner')
    ...
    for c1 in cont_var:
        for c2 in cont_var:
            ...
            if c1 != c2:
                ...
                t1 = [round(i,4) for i in mean_values_merged["diff with mean of response"].tolist()  if i not in [np.nan,float('Nan')]  ]
                fig = go.Figure(
                    data=go.Heatmap(                        
                ...
                cont_cont_brute_force_table_temp = pd.concat([cont_cont_brute_force_table_temp,cont_cont_brute_force_table],axis=0).reset_index(drop=True)
