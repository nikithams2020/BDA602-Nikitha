...
def main(logger):
    ...
    create_table(temp_df, "cramer_corr.html", click2, sort_by="corr")
    ...
    create_table(temp_df, "tschuprow_corr.html", click2, sort_by="corr")
    ...
    for c1 in cat_var:
        for c2 in cat_var:
            if c1 != c2:
                ...
                fig.write_html(file=filename, include_plotlyjs="cdn")
                try:
                    cramer_corr = co_rel.cat_correlation(c1, c2)
                except Exception:
                    cramer_corr = 0

                try:
                    tschuprow_corr = co_rel.cat_correlation(c1, c2, tschuprow=True)
    ...
    for c1 in cont_var:
        for c2 in cat_var:
            ...
            filename = f"{c1}_{c2}_cat_cont_brute_force"

            fig.write_html(file=filename, include_plotlyjs="cdn")
            try:
                corr_ratio = co_rel.cat_cont_correlation_ratio(c1, c2)
            except Exception:
                corr_ratio = 0
